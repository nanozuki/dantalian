...
impl<'a> Dantalian<'a> {
    ...
    pub async fn generate(&self, root: &str, force: &HashSet<String>) -> Result<()> {
        for e in WalkDir::new(root).min_depth(1).max_depth(1) {
            let entry = e?;
            if entry.file_type().is_dir() {
                let dir = dir::AnimeDir::new(entry).await?;
                let path_name = entry.path().to_str().ok_or()
                let job = job::Job::parse(entry.path(), &dir.Config, force.contains(&dir.path().
            }
        }
        Ok(())
    }
    ...
    pub async fn check_anime(&self, subject_id: u32) -> Result<AnimeData> {
        ...
        let data = AnimeData::from(bgm_anime);
        Ok(data)
    }
